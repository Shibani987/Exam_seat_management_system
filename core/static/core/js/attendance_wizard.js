...
// --- Generated Sheets Manager ---
function openGeneratedSheetsManager(){
  fetch('/get-generated-sheets/?t=' + Date.now())
    .then(r=>r.json()).then(d=>{
      if(d.status !== 'success') return alert('Failed to load generated sheets');
      const rows = d.sheets || [];
      const win = document.createElement('div');
      win.style.position='fixed'; win.style.left='6%'; win.style.top='4%'; win.style.width='88%'; win.style.height='88%'; win.style.background='#fff'; win.style.zIndex=99999; win.style.overflow='auto'; win.style.border='1px solid #ccc'; win.style.padding='12px'; win.style.boxShadow='0 8px 24px rgba(0,0,0,0.2)';
      const header = document.createElement('div'); header.style.display='flex'; header.style.justifyContent='space-between'; header.style.alignItems='center';
      const title = document.createElement('h3'); title.textContent='Generated Attendance Sheets'; header.appendChild(title);
      const closeBtn = document.createElement('button'); closeBtn.textContent='Close'; closeBtn.onclick = ()=> document.body.removeChild(win); header.appendChild(closeBtn);
      ...
      table.innerHTML = `<thead><tr style="text-align:left"><th style="width:40px">ID</th><th>Exam</th><th>File</th><th style="width:80px">Sheets</th><th style="width:100px">Students</th><th style="width:160px">Generated At</th><th style="width:220px">Actions</th></tr></thead>`;
      const tbody = document.createElement('tbody');

      rows.forEach(rw=>{
        const tr = document.createElement('tr'); tr.style.borderTop='1px solid #eee'; tr.style.padding='6px';
        const actionsTd = document.createElement('td'); actionsTd.style.padding='6px';
        tr.innerHTML = `<td style="padding:6px">${rw.id}</td><td style="padding:6px">${rw.exam_name}</td><td style="padding:6px">${rw.file_name}</td><td style="padding:6px">${rw.sheet_count}</td><td style="padding:6px">${rw.student_count}</td><td style="padding:6px">${rw.generated_at}</td>`;
        // actions
        const actTd = document.createElement('td'); actTd.style.padding='6px';
        const viewBtn = document.createElement('button'); viewBtn.textContent='View'; viewBtn.style.marginRight='8px';
        viewBtn.onclick = ()=>{
          fetch('/get-generated-sheet/?id=' + rw.id).then(rr=>rr.json()).then(j=>{
            if(j.status==='success'){
              // show Step3 preview with saved data
              document.getElementById('step1Content').style.display='none';
              document.getElementById('step2Content').style.display='none';
              document.getElementById('step3Content').style.display='block';
              document.getElementById('stepIndicator1').classList.remove('active');
              document.getElementById('stepIndicator2').classList.remove('active');
              document.getElementById('stepIndicator3').classList.add('active');
              generatedSheets = j.sheet.sheets || [];
              showStep3(generatedSheets, j.sheet.exam_name);
              document.body.removeChild(win);
            } else alert('Failed to load saved sheet');
          });
        };
        const printBtn = document.createElement('button'); printBtn.textContent='Print'; printBtn.style.marginRight='8px';
        printBtn.onclick = ()=>{
          fetch('/get-generated-sheet/?id=' + rw.id).then(rr=>rr.json()).then(j=>{
            if(j.status==='success'){
              generatedSheets = j.sheet.sheets || [];
        ...
        delBtn.onclick = ()=>{
          if(!confirm('Delete saved sheets for this exam? This cannot be undone.')) return;
          fetch('/delete-generated-sheet/',{method:'POST', headers:{'Content-Type':'application/json','X-CSRFToken': getCsrfToken()}, body:JSON.stringify({id: rw.id})}).then(rr=>rr.json()).then(j=>{
            if(j.status==='success'){ tr.remove(); }
            else alert('Delete failed: '+ (j.message||'error'));
          });
        };
    ...
    }).catch(err=>{console.error(err); alert('Failed to load generated sheets');});
}

// small floating manager button
(function(){
  const btn = document.createElement('button'); btn.textContent='Manage Generated Sheets'; btn.style.position='fixed'; btn.style.right='12px'; btn.style.bottom='12px'; btn.style.zIndex=9999; btn.style.padding='8px 12px'; btn.style.background='#1976d2'; btn.style.color='#fff'; btn.style.border='none'; btn.style.borderRadius='4px'; btn.style.cursor='pointer';
  btn.onclick = openGeneratedSheetsManager;
